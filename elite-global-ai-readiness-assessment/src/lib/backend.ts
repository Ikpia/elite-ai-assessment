...
// Central backend integration map for the frontend.
// Add or update backend endpoint wiring here instead of scattering route strings across UI files.
...
export const backendApi = {
  health: {
    check: () =>
      apiRequest<HealthResponse>(BACKEND_ENDPOINTS.health.check)
  },
  assessment: {
    ...
    status: (organisationId: string) =>
      apiRequest<OrganisationStatusResponse>(
        BACKEND_ENDPOINTS.assessment.status(organisationId)
      )
  },
  admin: {
    organisations: {
      list: (secret: string) =>
        apiRequest<OrganisationsListResponse>(
          BACKEND_ENDPOINTS.admin.organisations.list,
          {
      ...
      update: (
        ...
        apiRequest<UpdateOrganisationResponse>(
          BACKEND_ENDPOINTS.admin.organisations.update(organisationId),
          {
            method: "PATCH",
            headers: withAdminHeaders(secret),
            body: JSON.stringify(payload)
